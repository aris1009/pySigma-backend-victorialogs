"""End-to-end journald correctness harness.

Mirrors test_caddy_e2e.py but for the ``victorialogs_journald`` pipeline:

  1. Convert each rule via victorialogs_journald().
  2. POST the synthetic NDJSON dataset to /insert/jsonline (idempotent),
     stamping ``_time`` to now, mapping ``MESSAGE`` onto VL's ``_msg``
     (so unbound ``keywords:`` rules match), and tagging each event
...
Synthetic data is already in the journald pipeline-target shape, so no
...
from sigma.pipelines.victorialogs import victorialogs_journald
...
JOURNALD_EXPECTATIONS_PATH = REPO_ROOT / "e2e" / "expectations_journald.yml"
# Synthetic events carry SYSLOG_IDENTIFIER + dataset_label — combined
# they give every dataset its own VL stream.
JOURNALD_STREAM_FIELDS = "SYSLOG_IDENTIFIER,dataset_label"
# Public Linux Sigma rules under test are all `keywords:`-only filters
# that LogsQL resolves against `_msg`. Mapping `MESSAGE` onto `_msg`
# lets those keyword selectors match the synthetic free-form log line.
JOURNALD_MSG_FIELD = "MESSAGE"
...
def _convert(expectation: Expectation) -> str:
    ...
    backend = VictoriaLogsBackend(processing_pipeline=victorialogs_journald())
...
def _ensure_ingested(vl_url: str, dataset: Path, ingested_set: set[str]) -> None:
    ...
    ingest_jsonline_stamp_now(
        vl_url,
        dataset,
        stream_field=JOURNALD_STREAM_FIELDS,
        msg_field=JOURNALD_MSG_FIELD,
...
@pytest.mark.parametrize(
    "expectation", load_expectations(path=JOURNALD_EXPECTATIONS_PATH), ids=_expectation_id
)
def test_journald_rule_finds_expected_events(
