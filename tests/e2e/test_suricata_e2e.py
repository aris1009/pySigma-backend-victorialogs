"""End-to-end Suricata EVE correctness harness.

Mirrors test_caddy_e2e.py but for the ``victorialogs_suricata`` pipeline:

  1. Convert each rule via victorialogs_suricata().
...
The network/dns/proxy rules under test all use field-anchored selectors
on dns.rrname / http.url — no unbound keyword filters — so the harness
does not map any source field onto ``_msg``.
...
from sigma.pipelines.victorialogs import victorialogs_suricata
...
SURICATA_EXPECTATIONS_PATH = REPO_ROOT / "e2e" / "expectations_suricata.yml"
# EVE events carry `event_type` (flow/dns/http/tls/alert) — combined
# with `dataset_label` it gives every dataset its own VL stream.
SURICATA_STREAM_FIELDS = "event_type,dataset_label"
...
def _convert(expectation: Expectation) -> str:
    ...
    backend = VictoriaLogsBackend(processing_pipeline=victorialogs_suricata())
...
def _ensure_ingested(vl_url: str, dataset: Path, ingested_set: set[str]) -> None:
    ...
    ingest_jsonline_stamp_now(
        vl_url,
        dataset,
        stream_field=SURICATA_STREAM_FIELDS,
        msg_field=None,
...
@pytest.mark.parametrize(
    "expectation", load_expectations(path=SURICATA_EXPECTATIONS_PATH), ids=_expectation_id
)
def test_suricata_rule_finds_expected_events(
